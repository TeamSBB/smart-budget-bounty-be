...
public class UserServiceImpl implements UserService {
    ...
    public LoginDtoResponse loginUser(LoginDtoRequest loginUserDtoRequest) {
        ...
        User u = userRepo.findByEmail(userDetails.getUsername()); // Confusing part, the username is
                                                                  // the email
        ...
        if (account == null) {
            throw new EntityNotFoundException(
                "Account could not be found using userId: " + u.getId()
            );
        }
    ...
    public User getById(Long id) {
        LogUtil.logStart(logger, "Retrieving User by id.");
